...
module.exports = {
  async findOne(params, populate, { transacting } = {}) {
    const relatedCollections = this.associations
      ?.filter(assoc => assoc.targetUid && assoc.alias)
      ?.map(assoc => {
        if (
          ["belongsToMany", "manyWay"].includes(assoc.nature)
          && assoc.targetUid.startsWith("application::")
          && assoc.tableCollectionName 
          && assoc.tableCollectionName && this.allAttributes[assoc.alias].withPivot) {
          return { [assoc.alias]: (qb) => { qb.orderBy('_pivot_id', 'asc'); } }
        } else {
          return { [assoc.alias]: (qb) => qb }
        }
      }) ?? []

    const withRelated = this.uid.startsWith("application::") ? relatedCollections : populate;
  ...
  async update(params, { transacting } = {}) {
    ...
    const withRelated = this.uid.startsWith("application::") ? relatedCollections : undefined;
