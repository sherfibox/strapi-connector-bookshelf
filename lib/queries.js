...
module.exports = function createQueryBuilder({ model, strapi }) {
  ...
  function find(params, populate, { transacting } = {}) {
    ...
    const relatedCollections = model.associations
      ...
      ?.map(assoc => {
        if (
          ...
          && assoc.tableCollectionName && model.allAttributes[assoc.alias].withPivot ) {
    ...
    const withRelated = model.uid.startsWith("application::") ? relatedCollections : populate;
    ...
    return model
      ...
      .then(results => {
        ...
        if (res && params.id_in){
          const sanitize = [];
          for (const id of params.id_in){
            sanitize.push(res.find(item => item.id=== Number(id)));
          }
          res = sanitize;
        }
        return res;
